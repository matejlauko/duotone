...
// but for duotone, the root is in a different package, so
...
/**
 * @typedef {Object} GetUserViteConfig
 * @property {import('vite').UserConfig} userViteConfig;
 * @property {boolean} hasReactPlugin;
 * @property {boolean} hasTSConfigPathPlugin;
 */
 ...
 * @return {Promise<GetUserViteConfig>}
 */
export default async (command, mode, viteConfig) => {
  const userViteConfig = await loadConfigFromFile({ command, mode }, viteConfig).then(
